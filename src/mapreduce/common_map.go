...
func doMap(
	...
	content, err := io.ReadFile(inFile)
	if err != nil {
		debug("Fail to Read file %s", inFile)
		return
	}
	...
	for i := 0; i < nReduce; i++ {
		...
		if err != nil {
			debug("Fail to Create File %s", fname)
			return
		}
		fd = append(fd, file)
	}
	...
	debug("MAP Task %d Finish", mapTaskNumber)
}
